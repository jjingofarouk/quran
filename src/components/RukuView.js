...
function RukuView({ selectedRuku, setSelectedRuku, editions, settings, bookmarks, setBookmarks, notes, setNotes, progress, setProgress, setView }) {
  const [rukuData, setRukuData] = useState([]);
  const [offset, setOffset] = useState(0);
  const limit = 10; // Limit ayahs per page for pagination
  const rukuNumbers = Array.from({ length: 556 }, (_, i) => i + 1);

  useEffect(() => {
    if (selectedRuku) {
      const editionString = editions.join(',');
      const fetchRuku = async () => {
        try {
          const res = await fetch(`http://api.alquran.cloud/v1/ruku/${selectedRuku}/editions/${editionString}?offset=${offset}&limit=${limit}`);
          const data = await res.json();
          setRukuData(data.data);
          localStorage.setItem(`ruku_${selectedRuku}_${editionString}_${offset}`, JSON.stringify(data));
        } catch (e) {
          const cached = localStorage.getItem(`ruku_${selectedRuku}_${editionString}_${offset}`);
          if (cached) setRukuData(JSON.parse(cached).data);
        }
      };
      fetchRuku();
    }
  }, [selectedRuku, editions, offset]);
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="mb-4 flex space-x-4">
        <select
          onChange={(e) => {
            setSelectedRuku(e.target.value);
            setOffset(0);
          }}
          className="p-2 rounded border"
        >
          <option value="">Select Ruku</option>
          {rukuNumbers.map(ruku => (
            <option key={ruku} value={ruku}>Ruku {ruku}</option>
          ))}
        </select>
        <div>
          <button 
