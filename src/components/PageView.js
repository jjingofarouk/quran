...
function PageView({ selectedPage, setSelectedPage, editions, settings, bookmarks, setBookmarks, notes, setNotes, progress, setProgress, setView }) {
  ...
  useEffect(() => {
    if (selectedPage) {
      ...
      const fetchPage = async () => {
        try {
          const res = await fetch(`http://api.alquran.cloud/v1/page/${selectedPage}/editions/${editionString}?offset=${offset}&limit=${limit}`);
          const data = await res.json();
          setPageData(data.data);
          localStorage.setItem(`page_${selectedPage}_${editionString}_${offset}`, JSON.stringify(data));
        } catch (e) {
          const cached = localStorage.getItem(`page_${selectedPage}_${editionString}_${offset}`);
          if (cached) setPageData(JSON.parse(cached).data);
        }
      };
      fetchPage();
    }
  }, [selectedPage, editions, offset]);
