...
function HizbQuarterView({ selectedHizb, setSelectedHizb, editions, settings, bookmarks, setBookmarks, notes, setNotes, progress, setProgress, setView }) {
  ...
  useEffect(() => {
    if (selectedHizb) {
      ...
      const fetchHizb = async () => {
        try {
          const res = await fetch(`http://api.alquran.cloud/v1/hizbQuarter/${selectedHizb}/editions/${editionString}?offset=${offset}&limit=${limit}`);
          const data = await res.json();
          setHizbData(data.data);
          localStorage.setItem(`hizb_${selectedHizb}_${editionString}_${offset}`, JSON.stringify(data));
        } catch (e) {
          const cached = localStorage.getItem(`hizb_${selectedHizb}_${editionString}_${offset}`);
          if (cached) setHizbData(JSON.parse(cached).data);
        }
      };
      fetchHizb();
    }
  }, [selectedHizb, editions, offset]);
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="mb-4 flex space-x-4">
        <select
          onChange={(e) => {
            setSelectedHizb(e.target.value);
            setOffset(0);
          }}
          className="p-2 rounded border"
        >
          <option value="">Select Hizb Quarter</option>
          {hizbNumbers.map(hizb => (
            <option key={hizb} value={hizb}>Hizb Quarter {hizb}</option>
          ))}
        </select>
        <div>
          <button 
...
export default HizbQuarterView;